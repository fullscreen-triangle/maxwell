...
\end{equation}
The waking state requires continuous metabolic expenditure to maintain reality constraints.
\end{theorem}

\begin{proof}
Maintaining the reality constraint requires continuous comparison between internal configuration and sensory input, with variance minimization to align them. This comparison and alignment consume metabolic power $P_{\text{perception}} \approx 11$ W. When metabolic resources are redirected (as during sleep), the constraint cannot be maintained, and the system relaxes to unconstrained exploration.
\end{proof}
...
The phenomenon of lucid dreaming, in which the dreamer becomes aware that they are dreaming, can be characterised within this framework as partial reality injection.

\begin{definition}[Lucid State]
\label{def:lucid}
The lucid dreaming state is characterised by partial reality constraint:
\begin{equation}
...
The formation and retention of dream memories depend on the degree of reality injection at awakening.

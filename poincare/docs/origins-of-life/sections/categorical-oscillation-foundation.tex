...
\begin{figure*}[htbp]
\centering
\includegraphics[width=0.90\textwidth]{figures/categorical_oscillation_panel.png}
\caption{\textbf{Categorical Oscillation: Mathematical Foundation of Emergent Time.} \textbf{(A)} Three axioms: (1) Partitioning: $C_n = \sum_i C_{n,i}$ (red box)—categories can be subdivided; (2) Traversal: $C_n \to C_{n,1} \to C_{n,2} \to \cdots$ (blue box)—categories can be explored sequentially; (3) Recursion: $C_{n+1} = f(C_n, H_n)$ (green box)—next category depends on current category and history. These three axioms are necessary and sufficient for categorical oscillation. \textbf{(B)} Oscillation emergence: key insight is that $C_{n+1} \approx C_n$ (similar structure) BUT $C_{n+1} \neq C_n$ (different categorical state)—same quantity, different history creates oscillation. Difference arises due to partition history (purple text), not numerical change. \textbf{(C)} Partition sequence: four objects ($C_0 = 4$, blue circles) partition into $\{2,1,1\}$ (red pair, yellow, green), then return to $C_1 = 4$ with history $H_1 = \{2,1,1\}$, then partition into $\{3,1\}$ giving $C_2 = 4$ with history $H_2 = \{2,1,1\}, \{3,1\}$—same total, different history equals oscillation. \textbf{(D)} Electron transport as categorical oscillation: charge separation ↔ partitioning, electron movement ↔ traversal, field enables more ET ↔ recursion, same Q different history ↔ oscillation—electron transport instantiates categorical oscillation in physical systems. \textbf{(E)} Autocatalysis equals self-reference: system starts at center, loops back with growing partition history (blue spiral)—probability of electron transport increases with history: $P(\text{ET}|H_n) > P(\text{ET}|H_0)$. Self-reference creates temporal arrow through accumulating history. \textbf{(F)} Time emerges from sequence: categorical states $C_0 \to C_1 \to C_2 \to \cdots \to C_n$ (circles darkening from purple to green) with index $n$ as emergent time—temporal order defined by $H_{n-1} \subset H_n$ (history inclusion). Time is not fundamental dimension but emergent index of categorical sequence. Oscillation is not periodic return but categorical evolution with memory.}
\label{fig:categorical_oscillation}
\end{figure*}
...
\textbf{First partition}: Due to spin-orbit coupling (Theorem~\ref{thm:chiral_selection}), partitioning favours one chirality:
\begin{equation}

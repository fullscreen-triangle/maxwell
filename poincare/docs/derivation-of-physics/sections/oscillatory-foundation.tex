...
\caption{Oscillatory dynamics in bounded phase space. \textbf{Top row:} Bounded phase space with Poincar\'{e} recurrence (left); unbounded trajectory escaping (center-left); stability probability decay with phase space volume (center-right); energy surface visualization (right). \textbf{Middle row:} Classification of dynamics---static, monotonic, chaotic, and oscillatory---with only oscillatory satisfying all constraints. \textbf{Bottom row:} Frequency-energy identity $E = n\hbar\omega$; hierarchical timescale separation ($\sim 10^3$ ratio); recurrence time distribution; action quantization.}

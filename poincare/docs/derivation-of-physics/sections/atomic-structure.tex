...
\caption{Atomic structure from partition coordinates. \textbf{Top rows:} Periodic table with s/p/d blocks derived from partition geometry; shell filling order following $(n + l)$ rule; period lengths (2, 8, 8, 18, 18, 32, 32); Group 1 (alkali metals) ionization energies; 3d transition metal filling. \textbf{Middle rows:} Hydrogen spectrum (Lyman, Balmer, Paschen series); ionization energy periodic trend; electron configuration notation; atomic radius trend. \textbf{Bottom:} Electronegativity trend; Balmer series spectral lines; complete derivation chain from bounded phase space to periodic table.}
\label{fig:atomic}
\end{figure}

...
\caption{Spatial structure, matter, and energy. \textbf{Top row:} 3D emergence from angular coordinates $(l, m)$; radial scaling $r \propto n^2$; dimensionality uniqueness; locality from radial separation. \textbf{Middle row:} Occupied vs unoccupied modes (5\% occupation); exclusion principle from coordinate uniqueness; mass-frequency relationship $m = \hbar\omega/c^2$; cosmic mode occupation (dark sector). \textbf{Bottom row:} Wave-particle duality; energy conservation; Fermi-Dirac vs Bose-Einstein statistics; vacuum energy from unoccupied modes.}
\label{fig:spatial_matter}
\end{figure}

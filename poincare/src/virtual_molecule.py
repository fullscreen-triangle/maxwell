...
class CategoricalState:
    ...
    def from_hardware_timing(cls, 
        ...
        # The mapping: timing deviation → categorical position
        
        # S_k from the magnitude of deviation (how much we "know")
        S_k = min(1.0, abs(delta_p) * reference_freq)
        
        # S_t from the sign and fractional part (temporal position)
        S_t = (math.atan(delta_p * 1e9) / math.pi + 0.5)  # Maps to [0, 1]
        
        # S_e from the entropy of the deviation pattern
        # Use the bits of the timing value itself
        bits = abs(delta_p * 1e15) % 1.0  # Fractional nanoseconds
        S_e = bits

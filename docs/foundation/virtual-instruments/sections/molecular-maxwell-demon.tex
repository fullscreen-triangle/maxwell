...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/molecular_maxwell_demon_mass_spec_upper_half.png}
    \caption{\textbf{MMD framework architecture and coordinate system.}
    \textbf{(A)} Probability amplification cascade: dual filtering (input: experimental conditions; output: hardware constraints) amplifies probability from $\sim 10^{-11}$ (potential states) through $1.92 \times 10^9$ enhancement (input filter) and $2.00 \times 10^{-1}$ (output filter) to unity (actual observables).
    \textbf{(B)} S-entropy coordinate space: 14D feature space projects to 3D ($S_1$ mass, $S_2$ charge, $S_3$ energy) showing metabolite clustering.
    \textbf{(C)} Hardware oscillation hierarchy: 8-scale phase-lock architecture spans CPU clock (3.0 GHz) to system interrupts (10 Hz), grounding virtual measurements in physical computational substrates.
    \textbf{(D)} Virtual detector ensemble: mass resolution comparison shows TOF ($2 \times 10^4$), Orbitrap ($10^6$), FT-ICR ($10^7$), and IMS ($10^4$) projections from single categorical state.}
...
\textbf{Input Philtre ($\Im_{\text{input}}$):} Selects from the space of potential molecular configurations $Y_{\downarrow}^{(\text{in})}$ those states that are compatible with specified experimental conditions:
...
\textbf{Output Philtre ($\Im_{\text{output}}$):} Validates the physical realisability of potential observables $Z_{\downarrow}^{(\text{fin})}$ against hardware coherence constraints:
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/molecular_maxwell_demon_mass_spec_middle.png}
    \caption{\textbf{Categorical completion and post-hoc reconfiguration.}
    \textbf{(E)} Example mass spectrum for metabolite ID 0 shows single peak at m/z $\approx 945$ with intensity $\approx 700$.
    \textbf{(F)} Categorical completion: original peaks (green) at m/z 150, 250, 500 augmented with virtual peaks (purple) at m/z 300, 550, demonstrating MMD completion of missing spectral features.
    \textbf{(G)} Post-hoc reconfiguration: virtual experimental conditions (purple) modified from original (green) for temperature ($300 \to 350$ K), collision energy ($\sim 25 \to 40$ eV), and ionization method, enabling retrospective method optimization without re-measurement.
    \textbf{(H)} Multi-instrument projections: single categorical state projects to TOF, Orbitrap, FT-ICR, and IMS virtual detectors simultaneously.}

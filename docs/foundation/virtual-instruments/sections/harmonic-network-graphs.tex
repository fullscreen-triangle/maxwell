...
A fragmentation tree $\mathcal{T}_{\text{mol}}$ for molecule $M$ is a directed acyclic graph (DAG) that is, in fact, a tree:
...
The finite observer method (Section 4) detects phase-locks between molecular oscillations and hardware references. A profound consequence \textit{is that ions from different molecules can be harmonically coupled}.
...
\textbf{View 2 (Top-Down):} $(S\text{-Knowledge},\, S\text{-Time})$ projection showing identical temporal ordering pattern. Early fragments cluster at $S\text{-Time} \approx 0.05\text{--}0.10$, late fragments at $S\text{-Time} \approx 0.30\text{--}0.35$. The diagonal progression rate $\left(\frac{\partial S\text{-Knowledge}}{\partial S\text{-Time}} = 18.3 \pm 2.1\right)$ matches Waters data $(17.9 \pm 1.8)$ within statistical error, confirming platform-invariant progression dynamics.\\
\textbf{View 3 (Side):} $(S\text{-Time},\, S\text{-Entropy})$ projection revealing identical entropy decay profile. Exponential fit yields decay constant $\lambda = 6.8 \pm 0.4$, statistically indistinguishable from Waters data $\bigl(\lambda = 7.1 \pm 0.5,\ p = 0.62\bigr)$. This proves that oscillatory termination probability is instrument-independent.
